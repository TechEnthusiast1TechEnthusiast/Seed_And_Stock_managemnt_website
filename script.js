document.addEventListener('DOMContentLoaded', function () {
    ...
    // Function to update farmer details summary
    function updateFarmerSummary() {
    ...
    function saveBill() {
        // Get all relevant data from the billing form
        let billDate = billDateInput.value;
        let billItemsHTML = billSummaryBody.innerHTML;

        // Create a new row in the "Bills Saved" section
        let savedBillsTable = document.getElementById('saved-bills-table');
        let newRow = savedBillsTable.insertRow();

        // Populate the new row with bill data
        let cell1 = newRow.insertCell(0);
        cell1.innerHTML = billDate;

        let cell2 = newRow.insertCell(1);
        cell2.innerHTML = billItemsHTML;

        // Clear the bill array after saving
        bill = [];

        // Clear the bill summary table
        billSummaryBody.innerHTML = '';
    ...
    // Event listener for the Save Bill button
    document.getElementById('save-bill').addEventListener('click', function () {
        saveBill();
    });

    // Function to handle bill search
    document.getElementById('btn-search').addEventListener('click', function () {
        let searchValue = document.getElementById('bill-search').value.trim().toLowerCase();
        let table = document.getElementById('saved-bills-table');
